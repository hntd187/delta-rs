...
impl ConstraintBuilder {
    ...
    /// Specify a constraint to be added
    pub fn with_constraint<S: Into<String>, E: Into<Expression>>(
        mut self,
        name: S,
        expression: E,
    ) -> Self {
        self.constraints.push((name.into(), expression.into()));
        self
    }

    /// Specify multiple constraints to be added. These will be added in the order they are in the vec
    pub fn with_constraints<S: Into<String>, E: Into<Expression>>(
        mut self,
        constraints: Vec<(S, E)>,
    ) -> Self {
        let build_constraints: Vec<_> = constraints.into_iter().map(|(k, v)| (k.into(), v.into())).collect();
        self.constraints.extend(build_constraints);
        self
    }
...
impl std::future::IntoFuture for ConstraintBuilder {
    ...
    fn into_future(self) -> Self::IntoFuture {
        ...
        Box::pin(async move {
            ...
            for (name, expr) in this.constraints {
...
mod tests {
    ...
    #[tokio::test]
    async fn add_multiple_valid_constraint() -> DeltaResult<()> {
        let batch = get_record_batch(None, false);
        let write = DeltaOps(create_bare_table())
            .write(vec![batch.clone()])
            .await?;
        ...
        let op_params = &table.metadata()?.configuration;
        let first_c = op_params.get("delta.constraints.id").unwrap().as_ref().unwrap();
        let second_c = op_params.get("delta.constraints.id2").unwrap().as_ref().unwrap();

        assert_eq!(first_c, "value < 1000");
        assert_eq!(second_c, "value > -1000");
        Ok(())
    }
